...
data "aws_instances" "datas" {
  depends_on = [ "aws_autoscaling_group.data_nodes" ]
...
resource "aws_ebs_snapshot" "elastic_data_snapshot" {
  count       = var.datas_count
  volume_id = aws_ebs_volume.elastic_data.*.id[count.index]

  tags = {
    Name = "Elastic logging clustter snapshot"
  }
}